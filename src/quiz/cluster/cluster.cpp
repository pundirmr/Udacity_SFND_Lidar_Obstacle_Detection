...
// Helper function to recursively expand clusters
...
{
    // Mark point as processed and add it to the current cluster
    processed[indice] = true;
    cluster.push_back(indice);

    // Find nearby points within distance tolerance using KdTree search
    std::vector<int> nearbyPoints = tree->search(points[indice], distanceTol);

    // Recursively expand cluster for each nearby point
...
// Main function for clustering points using Euclidean distance
std::vector<std::vector<int>> euclideanCluster(const std::vector<std::vector<float>>& points, KdTree* tree, float distanceTol)
{
    // Vector to store clusters
    std::vector<std::vector<int>> clusters;

    // Vector to keep track of processed points
    ...
    for (int i = 0; i < points.size(); ++i)
    {
        ...
        // Expand cluster from current point
